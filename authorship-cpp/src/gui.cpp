...
static std::vector<std::string> g_blockTabNames;
...
static PairSeverityFilter g_pairSeverityFilter = SEV_FILTER_ALL;
...
// SEV_FILTER_ALL always passes.
static bool pairMatchesSeverityFilter(double combinedScorePct, PairSeverityFilter f) {
...
// different pair once the visible list is re-numbered.
static std::set<int> g_expandedPairs;
...
static std::set<int> g_showAllFeaturesKeys;
...
static int   g_copyFindingsFeedbackPairIdx = -1;
static DWORD g_copyFindingsFeedbackUntilTick = 0;
...
// scoreColor() above is signed for AUTHORSHIP LIKELIHOOD (high = matches
...
static int drawResultsHeader(HDC hdc, int x, int y, int width) {
    int startY = y;
                                      ...
                                      WPARAM wParam, LPARAM lParam)
{
    switch (uMsg) {
        ...
        case WM_MOUSEMOVE: {
            // Style DNA strip hover (spec §4.4: detail-page strip is
            // "interactive on hover"). Guarded on g_currentPage/
            // g_selectedStudent rather than just checking the hit
            // buffer non-empty — that buffer can go briefly stale
            // when navigating away from the detail page via a path
            // that doesn't repaint it (e.g. switching tabs directly),
            // so this is the one check guaranteed correct regardless
            // of which navigation path was taken.
        ...
        case WM_LBUTTONDOWN: {
            ...
            // Block sub-tab click? (Classes page grid view only — fixed
            // Y zone right below the main tab bar, not scrolled.)
            // BLOCK_TAB_BAR_HEIGHT scaled here for consistency with its
            // siblings in this expression even though drawBlockTabBar's
            // own internal drawing isn't scaled until the Classes batch
            // — see that batch's report for the resulting temporary gap.
            if (g_currentPage == PAGE_STUDENTS && g_selectedStudent < 0 &&
                my >= S(4) + S(TAB_BAR_HEIGHT) + S(CONTEXT_STRIP_HEIGHT) &&
            ...
            // Classes grid: Sort control click — cycles Name →
            // Submissions → Flagged → Name (spec §5.8).
            if (g_currentPage == PAGE_STUDENTS && g_selectedStudent < 0 &&
            ...
            // Overview page: check for Exact Duplicates card click
            if (g_currentPage == PAGE_OVERVIEW &&
                g_analysisResults.exactDuplicates > 0)
            {
            ...
            // Student detail page: Flagged Appearances rows are
            // clickable, deep-linking to Flagged Pairs (spec §5.8).
            // Auto-expands the target pair now that Flagged Pairs has
            // an expand/collapse model (step 8) — previously this just
            // switched tabs and left the instructor to find the pair
            // themselves in a fully-expanded, unfiltered list.
            if (g_currentPage == PAGE_STUDENTS && g_selectedStudent >= 0 &&
                !g_flaggedAppearanceRowRects.empty())
            {
            ...
            // AI summary pair deep-links (spec §5.7) — the single
            // highest-value interaction the brief calls out. Now also
            // auto-expands the referenced pair (see comment above —
            // same step-8 dependency, now resolved).
            if (g_currentPage == PAGE_OVERVIEW && !g_aiSummaryLinkRects.empty()) {
            ...
            // Overview page: language notice "Show files" toggle
            if (g_currentPage == PAGE_OVERVIEW &&
                !g_analysisResults.languageMismatchFiles.empty() &&
            ...
            // Overview page: check for Unclassified file Assign/Delete clicks
            if (g_currentPage == PAGE_OVERVIEW && !g_unclassifiedActionRects.empty()) {
            ...
            // Flagged Pairs page: collapsed row click → expand (spec §5.9)
            if (g_currentPage == PAGE_FLAGGED && !g_collapsedPairRowRects.empty()) {
                bool hitRow = false;
            ...
            // Flagged Pairs page: expanded block's header click → collapse
            if (g_currentPage == PAGE_FLAGGED && !g_expandedPairHeaderRects.empty()) {
                ...
                if (hitHeader) return 0;
            }

            // Flagged Pairs page: "Show all 14 features" disclosure toggle
            if (g_currentPage == PAGE_FLAGGED && !g_showAllFeaturesRects.empty()) {
                bool hitDisc = false;
            ...
            // Flagged Pairs page: "Copy findings" button
            if (g_currentPage == PAGE_FLAGGED && !g_copyFindingsBtnRects.empty()) {
                bool hitCopy = false;
            ...
            // Flagged Pairs page: severity filter chip click (spec §5.9)
            if (g_currentPage == PAGE_FLAGGED && !g_severityChipRects.empty()) {
                bool hitChip = false;
            ...
            // Flagged Pairs page: check for clear-filter button
            if (g_currentPage == PAGE_FLAGGED && g_filterExactDuplicatesOnly) {
            ...
            // Students page: card click → open detail, OR back button → close detail
        ...
        case WM_PAINT: {
            ...
            if (g_state.analysisComplete && g_analysisResults.valid) {
                ...
                if (g_currentPage == PAGE_OVERVIEW) {
                    ...
                    if (g_selectedStudent >= 0) {
                        ...
                        totalUsed += drawStudentDetailPage(hdc, contentX,
                                                            contentY + totalUsed,
                                                            contentW,
                                                            g_selectedStudent);
                    } else {
                        totalUsed += drawStudentProfiles(hdc, contentX, contentY + totalUsed, contentW);
            ...
            // Style DNA hover tooltip (spec §4.4)
            if (g_currentPage == PAGE_STUDENTS && g_selectedStudent >= 0 &&
                g_dnaHoveredCell >= 0 && g_dnaHoveredCell < (int)g_dnaCellHits.size())
            {
                ...
                // Position above the cursor, clamped so it never runs
                // off the top or right edge of the content area.
